...
class CompilerPlugin(plugins.BasePlugin.BasePlugin, object):
    ...
    def snippet(self, data, extra_args):
        ...
        cpp_file = open(code_filename, "w+")
        for prefix_file in extra_args.get("prefix_files", None):
            cpp_prefix_file = open(prefix_file, "r")
            cpp_file.write(cpp_prefix_file.read())
            cpp_prefix_file.close()
            cpp_file.write("\n")

        cpp_file.write(data["command"])

        for suffix_file in extra_args.get("suffix_files", None):
            cpp_suffix_file = open(suffix_file, "r")
            cpp_file.write(cpp_suffix_file.read())
            cpp_suffix_file.close()
            cpp_file.write("\n")
        cpp_file.flush()
        cpp_file.close()
